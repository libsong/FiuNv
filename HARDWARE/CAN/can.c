...
uint32_t    g_RemoteCanId = 0x1FFFFFFF;	//对端
uint32_t    g_LocalCanId_r = 0x1FFFFFFF; //本地
...
//    printf("len:%d\r\n",CAN1_Handler.pRxMsg->DLC);
	if (CAN1_Handler.pRxMsg->ExtId == g_LocalCanId_r) {
		if(((g_IntrDataCur + 8) % CANBUFDATALEN) != g_DealDataCur)
		{
			memcpy(&g_HOSTCANDATABUF[g_IntrDataCur],CAN1_Handler.pRxMsg->Data,8);

			g_IntrDataCur += 8;
			g_IntrDataCur %= CANBUFDATALEN;
		}
	}    
}
#endif	
...
{	
    ...
    CAN1_Handler.pTxMsg->StdId = 0;        //标准标识符
    CAN1_Handler.pTxMsg->ExtId = g_RemoteCanId;        //扩展标识符(29位)
    //CAN1_Handler.pTxMsg->IDE=CAN_ID_STD;    //使用标准帧
	CAN1_Handler.pTxMsg->IDE=CAN_ID_EXT;
	...
	rv = HAL_CAN_Transmit(&CAN1_Handler,10);
    if(rv != HAL_OK) 
		return 1;     //发送
