...
TIM_HandleTypeDef TIM10_Handler;//定时器句柄 ，用来控制故障激活时间
...
{  
    ...
    TIM2_Handler.Init.Prescaler=psc-1;                     //分频系数
    TIM2_Handler.Init.CounterMode=TIM_COUNTERMODE_UP;    //向上计数器
    TIM2_Handler.Init.Period=arr-1;                        //自动装载值
    TIM2_Handler.Init.ClockDivision=TIM_CLOCKDIVISION_DIV1;//时钟分频因子
    HAL_TIM_Base_Init(&TIM2_Handler);
    
	__HAL_TIM_CLEAR_IT(&TIM2_Handler, TIM_IT_UPDATE);//避免定时器 一开启立即进入中断
...
{ 
    ...
    TIM10_Handler.Init.Period=arr-1;          //自动重装载值
    ...
    TIM10_CH1Handler.Pulse=ccr-1;            //设置比较值,此值用来确定占空比，默认比较值为自动重装载值的一半,即占空比为50%
