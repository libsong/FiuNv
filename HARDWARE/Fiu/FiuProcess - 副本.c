...
u8 uFaultModePreStatus,uFaultModeNowStatus,uFaultModeStatus;  // 0x0:∏¥∫œπ ’œ£¨1£∫øÏÀŸÕ®µ¿ 2£∫øÏÀŸ¬©µÁ 3£∫¥ÛµÁ¡˜
extern u8 udp_demo_recvbuf[2000];	//UDPΩ” ’ ˝æ›ª∫≥Â«¯ 
...
{
	 u8 i;
	 for(i=0;i<14;i++)   //≥ı ºªØ SN74374 £¨…ËŒ™0
	{
   Sn74374_Clear0(i);
	}	
...
void fuhegz_set(u8 uChannel,u8 uSetValue)   //uChannel:0-71
{
  ...
  if(uSetValue==2)  // ECU-UBATA+ ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
	...
	else if(uSetValue==3)  // ECU-UBATA- ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
	...
	else if(uSetValue==4)  // ECU-UBATB+ ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
	...
	else if(uSetValue==5)  // ECU-UBATB- ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
...
void ddlgz_set(u8 uChannel,u8 uSetValue)   //uChannel:0-71
{
  ...
  if(uSetValue==2)  // ECU-UBATC+ ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
	...
	else if(uSetValue==3)  // ECU-UBATC- ∂Ã¬∑π ’œ
  {
    utemp=uChannel*3+1;
	  uDqNumber=utemp/16;   
	  uDqPin=utemp%16;
...
void kstdgz_set(u8 uChannel,u8 uSetValue)   //uChannel:0-71
{
	...
	if(uSetValue==1)   //ECU-LOADø™¬∑π ’œ
	{
		HV_FunRelay_Set(9); // πÃÃ¨ºÃµÁ∆˜∂œø™
	}	
	else if(uSetValue==2)  // ECU-UBATA+ ∂Ã¬∑π ’œ
  {
   	HV_FunRelay_Set(5); // πÃÃ¨ºÃµÁ∆˜±’∫œ
	}		
	else if(uSetValue==3)  // ECU-UBATA- ∂Ã¬∑π ’œ
  {
    HV_FunRelay_Set(6); // πÃÃ¨ºÃµÁ∆˜±’∫œ
	}		
	else if(uSetValue==4)  // ECU-UBATB+ ∂Ã¬∑π ’œ
  {
   	HV_FunRelay_Set(7); // πÃÃ¨ºÃµÁ∆˜±’∫œ
	}		
	else if(uSetValue==5)  // ECU-UBATB- ∂Ã¬∑π ’œ
  {
    HV_FunRelay_Set(8); // πÃÃ¨ºÃµÁ∆˜±’∫œ
	}		
...
void ksldmn_set(u8 uChannel,u8 uSetValue)   //uChannel:0-71
{
	...
	if(uSetValue==1)   //ECU-LOAD¬©µÁƒ£ƒ‚
	{
		HV_FunRelay_Set(10); // πÃÃ¨ºÃµÁ∆˜±’∫œ
		Resistance_select(uFiuCom_buf[13],uFiuCom_buf[14]);
	}	
	else if(uSetValue==2)  // ECU-UBATA+ ¬©µÁƒ£ƒ‚
  {
   	HV_FunRelay_Set(5); // πÃÃ¨ºÃµÁ∆˜±’∫œ
		Resistance_select(uFiuCom_buf[13],uFiuCom_buf[14]);
	}		
	...
	else if(uSetValue==5)  // ECU-UBATB- ∂Ã¬∑π ’œ
  {
	...
	else if(uSetValue==7)  // ECU-LOAD- µÁ¡˜
  {
    HV_FunRelay_Set(11); // πÃÃ¨ºÃµÁ∆˜±’∫œ
		Resistance_select(uFiuCom_buf[13],uFiuCom_buf[14]);
	}		
 ...
 for (i=0;i<17;i++)
	...
	if((uFiuCom_buf[0]==0xbe) && (uFiuCom_buf[1]==0xbe) && (uFiuCom_buf[15]==0xed) && (uFiuCom_buf[16]==0xed))
	{
		...
		switch (uFiuCom_buf[2])
			{
        case 0:fuhegz_process();break;  //∏¥∫œπ ’œ
        case 1:kstdgz_process();break;  //øÏÀŸÕ®µ¿π ’œ
        case 2:ksldmn_process();break;  //øÏÀŸ¬©µÁƒ£ƒ‚
        case 3:ddltdgz_process();break;  //¥ÛµÁ¡˜π ’œ
	...
	//HAL_UART_Transmit(&UART1_Handler,uFiuCom_buf,14,1000);	//∑¢ÀÕΩ” ’µΩµƒ ˝æ›
	//while(__HAL_UART_GET_FLAG(&UART1_Handler,UART_FLAG_TC)!=SET);		//µ»¥˝∑¢ÀÕΩ· ¯	
...
void HI_FunRelay_Set(u8 uSetValue)
{
  if (uSetValue==0)  //∏¥Œª
	...
	else if(uSetValue==1)  //∏¥∫œπ ’œ,∂‘UBATC+
	{
	  CTRLBUS2_RESET;
		CTRLBUS14_RESET;
		CTRLBUS15_RESET;
	...
	else if(uSetValue==2)  //∏¥∫œπ ’œ,∂‘UBATC-
	{
	  CTRLBUS2_RESET;
		CTRLBUS14_RESET;
		CTRLBUS15_RESET;
...
void HV_FunRelay_Set(u8 uSetValue)
{
  if (uSetValue==0)  //∏¥Œª
	...
	else if(uSetValue==1)  //∏¥∫œπ ’œ,∂‘UBATA+
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_SET;		
	...
	else if(uSetValue==2)  //∏¥∫œπ ’œ,∂‘UBATA-
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_RESET;		
	...
	else if(uSetValue==3)  //∏¥∫œπ ’œ,∂‘UBATB+
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS12_SET;		
	...
	else if(uSetValue==4)  //∏¥∫œπ ’œ,∂‘UBATB-
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS12_RESET;		
	...
	else if(uSetValue==5)  //øÏÀŸ«–ªª,∂‘UBATA+
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_SET;		
	...
	else if(uSetValue==6)  //øÏÀŸ«–ªª,∂‘UBATA-
	{
	CTRLBUS1_SET;
  CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_RESET;		
	...
	else if(uSetValue==7)  //øÏÀŸ«–ªª,∂‘UBATB+
	{
	CTRLBUS1_SET;
  CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS12_SET;		
	...
	else if(uSetValue==8)  //øÏÀŸ«–ªª,∂‘UBATB-
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_RESET;		
	...
	else if(uSetValue==9)  //øÏÀŸ«–ªª,∂‘ECU OPEN TO LOAD
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_RESET;		
	...
	else if(uSetValue==10)  //øÏÀŸ«–ªª,∂‘ECU CONNECT TO LOAD
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_RESET;	
  ...
  CTRLBUS13_RESET;		
	...
	else if(uSetValue==11)  //øÏÀŸ«–ªª,µÁ¡˜≤‚¡ø
	{
	CTRLBUS1_SET;
	CTRLBUS2_RESET;	
	CTRLBUS3_SET;	
...
void RelayIO_Init(void)
{
    ...
    __HAL_RCC_GPIOB_CLK_ENABLE();           //ø™∆ÙGPIOB ±÷”
	  ...
	  __HAL_RCC_GPIOD_CLK_ENABLE();           //ø™∆ÙGPIOD ±÷”
	  __HAL_RCC_GPIOE_CLK_ENABLE();           //ø™∆ÙGPIOD ±÷”
	  __HAL_RCC_GPIOF_CLK_ENABLE();           //ø™∆ÙGPIOD ±÷”
	  __HAL_RCC_GPIOG_CLK_ENABLE();           //ø™∆ÙGPIOD ±÷”
    __HAL_RCC_GPIOH_CLK_ENABLE();           //ø™∆ÙGPIOH ±÷”
	  __HAL_RCC_GPIOI_CLK_ENABLE();           //ø™∆ÙGPIOI ±÷”
    ...
    GPIO_Initure.Mode=GPIO_MODE_INPUT;      // ‰»Î
    GPIO_Initure.Pull=GPIO_PULLDOWN;          //œ¬¿≠
    GPIO_Initure.Speed=GPIO_SPEED_HIGH;     //∏ﬂÀŸ
    HAL_GPIO_Init(GPIOH,&GPIO_Initure);
